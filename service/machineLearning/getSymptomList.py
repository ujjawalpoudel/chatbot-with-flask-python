...
def recurse_node_depth(tree_predict, feature_name, disease_input, node=0, depth=1):
    ...
    if tree_predict.feature[node] != _tree.TREE_UNDEFINED:
        ...
        if name == disease_input:
            ...
            return recurse_node_depth(tree_predict, feature_name, disease_input, tree_predict.children_right[node], depth + 1)
        else:
            # If not, traverse the left child node to continue diagnosing
            return recurse_node_depth(tree_predict, feature_name, disease_input, tree_predict.children_left[node], depth + 1)
    else:
